...
def get_password_policy_status(): 
    try:
        ...
        output = clean_output(output)  # 🧼 Clean the weird chars
        ...
        if "Length of password history maintained:" in output:
            password_history = output.split("Length of password history maintained:")[1].split("\n")[0].strip()

        if min_length == 0:
            return f"Weak (Users can set empty passwords)\n{output}"
        elif password_history.lower() == "none":
