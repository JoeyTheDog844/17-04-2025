...
def calculate_security_score():
    score = 100
    deductions = []
    password_policy = get_password_policy_status()
    lockout_policy = get_lockout_policy_status().lower()
    ...
    if "Weak" in password_policy:
        score -= 15
        deductions.append(("Weak Password Policy", -15))
    elif "Partial" in password_policy:
        score -= 7
        deductions.append(("Partial Password Policy", -7))

    if "No System Lockout Policy" in lockout_policy:
        score -= 10
    ...
    # ⚙️ System Info
    if "Manual" in pnp_status:
        score -= 2
        deductions.append(("Plug and Play Set to Manual", -2))
    elif "Disabled" in pnp_status:
        score -= 5
        deductions.append(("Plug and Play is Disabled", -5))
